function m_selectProfile(hObject, profileSelection)
    ...
    lineTool = uitoggletool(ht, 'CData', line_icon, ...
        'TooltipString', 'Select a Line Profile', ...
        'State','on', ...
        'tag','line_profile');
    rectangleTool = uitoggletool(ht, 'CData', rectangle_icon, ...
        'TooltipString', 'Select a Region Profile', ...
        'tag','rectangle_profile');
    ellipseTool = uitoggletool(ht, 'CData', ellipse_icon, ...
        'TooltipString','Select a Ellipse Region Profile', ...
        'tag','ellipse_profile', ...
        'separator','on');
    minusTool = uipushtool(ht,'CData', minus_icon, ...
        'tooltipstring','Reduce thickness of circle/ellipse', ...
        'tag','ellipse_profile_minus_button', ...
        'enable', 'off');
    plusTool = uipushtool(ht,'CData', plus_icon, ...
        'tooltipstring','Increase thickness of circle/ellipse', ...
        'tag','ellipse_profile_plus_button', ...
        'enable','off');
    ...
    set(minusTool, ...
        'clickedCallback', {@clickedminusEllipseTool});
    set(plusTool, ...
        'clickedCallback', {@clickedplusEllipseTool});
    ...
    % check if we can validate or not the + and - buttons
    checkMinusPlusButton();
    ...
    function formatSelection(profileSelection)
        ...
        szProfile = size(profileSelection,1);
        ...
        switch workingProfile
            ...
            case 'line'
                ...
                if ~strcmp(pointer,'crosshair')
                    ...
                    point1 = [profileInitialPosition(1) profileInitialPosition(2)];
                    point2 = [profileInitialPosition(3) profileInitialPosition(4)];
    ...
    %% check if the Minus and Plus button for the ellipse can be enabled
    function checkMinusPlusButton()
        
        if isempty(roiEllipse)
            activeStatus = 'off';
        else
            activeStatus = 'on';
        end
        
        set(minusTool,'enable',activeStatus);
        set(plusTool,'enable',activeStatus);
    ...
    function RoiRefreshImage (~)
        ...
        if sz > 0
            for i=1:sz
                ...
                %display where the profile will start
                xArrow = tmpRoiEllipse(1) + tmpRoiEllipse(3)/2;
                yArrow = tmpRoiEllipse(2);
                hold on
                plot(xArrow,yArrow,'>','markersize',50,...
                    'markerfacecolor','r');
    ...
    function hFigure_closeRequestFcn(hFigure, ~, hObject)
        ...
        if get(handles.listboxBAprofile,'value') == 0
            set(handles.listboxBAprofile,'value',1);
        end
        
        set(handles.listboxBAprofile,'string', formatedSelection);
    ...
    function clickedminusEllipseTool(~,~)
        %  Trigger by the minus button when the
        % ellipse mode is activated. Will decrease the size of the
        % circle border
        if ellipseThickness > 1
            ellipseThickness = ellipseThickness - 1;
            RoiRefreshImage(hObject);
        end
    end
    
    function clickedplusEllipseTool(~,~)
        %  Trigger by the plus button when the
        % ellipse mode is activated. Will increase the size of the
        % circle border
        ellipseThickness = ellipseThickness +1;
        RoiRefreshImage(hObject);
    end
